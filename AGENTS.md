...
**The colour tokens are Material Design 3.** Every shadcn token in `app/globals.css` reads an `--md-sys-color-*` role, and [`material-theme-builder`](https://github.com/abernier/material-theme-builder) derives all of them from one source hex. Retuning the palette means changing that hex — or the `scheme` / `contrast` / core-colour overrides next to it in `bin/gen-mcu-css.mjs` — never editing a token by hand. Anything the m3 roles don't cover belongs in that file's `customColors`, which mints `--md-sys-color-<name>` and a matching `-on-` foreground.

The palette is **generated, not imported at runtime**. `bin/gen-mcu-css.mjs` writes `app/mcu.generated.css` (gitignored) and both `dev` and `build` run it first, so the colours are compiled into the stylesheet and are already there in the prerendered HTML. Do not swap it for the package's `<Mcu>` component: that fills its `<style>` tag from `useInsertionEffect`, and this app is `output: "export"` — the browser would paint the whole page before a single colour existed. The same reason rules out calling `builder()` from the layout, though there it fails outright: the package's dist carries a hoisted `"use client"`, so a Server Component gets `Attempted to call builder() from the server`.
