...
type TwentyGLTFResult = GLTF & {
  ...
  materials: {
    boxBase: THREE.MeshStandardMaterial;
...
type HatGLTFResult = GLTF & {
  ...
  materials: {
    ...
    boxCap: THREE.MeshStandardMaterial;
  };
};
...
function Scene(props: ThreeElements["group"]) {
  const { nodes, materials } = useGLTF(
    twentyModel,
  ) as unknown as TwentyGLTFResult;
  return (
...
function Hats({
  ...
  const { nodes, materials } = useGLTF(hatModel) as unknown as HatGLTFResult;
  const instances: InstancedRigidBodyProps[] = Array.from(
    { length: count },
    (_, i) => ({
      key: i,
