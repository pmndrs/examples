...
export const Heart = () => {
  ...
  const controlPoints1 = [new THREE.Vector3(0, 0, 0), new THREE.Vector3(1, 1, 0), new THREE.Vector3(1, 3, 0), new THREE.Vector3(0, 2, 0)].map((point) =>
    point.multiplyScalar(2)
  )
  const controlPoints2 = controlPoints1.map((point) => new THREE.Vector3(-point.x, point.y, point.z))
  return [controlPoints1, controlPoints2].map(([v0, v1, v2, v3], index) => <cubicBezierCurve3 key={index} v0={v0} v1={v1} v2={v2} v3={v3} />)
}
...
export const Infinity = () => {
  ...
  for (let i = 0; i < segments; i++) {
    ...
    const startPoint = new THREE.Vector3(centerX + radius * Math.cos(startAngle), centerY + amplitude * Math.sin(2 * startAngle), radius * Math.sin(startAngle))
    const endPoint = new THREE.Vector3(centerX + radius * Math.cos(endAngle), centerY + amplitude * Math.sin(2 * endAngle), radius * Math.sin(endAngle))
