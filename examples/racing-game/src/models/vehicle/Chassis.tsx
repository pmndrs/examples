...
export const Chassis = forwardRef<Group, ChassisProps>(
  ({ args = [2, 1.1, 4.7], mass = 500, children, ...props }, ref) => {
    ...
    const [camera, ready, set, vehicleConfig] = useStore((s) => [
      s.camera,
      s.ready,
      s.set,
      s.vehicleConfig,
    ...
    const { nodes: n, materials: m } = useGLTF(
      assetUrl("models/chassis-draco.glb"),
    ) as unknown as ChassisGLTF;
    ...
    useFrame((_, delta) => {
      ...
      brake.current.material.opacity = MathUtils.lerp(
        brake.current.material.opacity,
        controls.brake ? 1 : 0.3,
        delta * 10,
      );
      glass.current.material.opacity = MathUtils.lerp(
