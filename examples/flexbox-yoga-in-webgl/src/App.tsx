...
import state, { type ContentEntry } from "./state";

function HeightReporter({
  onReflow,
}: {
  onReflow?: (width: number, height: number, centerAnchor?: boolean) => void;
}) {
...
function Content({ onReflow }: { onReflow: (pages: number) => void }) {
...
export default function App() {
  ...
  const onScroll = (e: { target: EventTarget }) =>
    (state.top = (e.target as HTMLDivElement).scrollTop);
  ...
  return (
    <>
      <Canvas
        shadows
        /* `enabled` is an r3f v8 raycaster flag that v9 dropped from the prop type */
        raycaster={{ enabled: false } as Partial<THREE.Raycaster>}
        dpr={[1, 2]}
