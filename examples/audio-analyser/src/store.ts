...
// Placeholder used before the real analyser data has loaded; the missing
// AudioState members (source/update/setGain) are never read before loaded()
// replaces this with the resolved createAudio() result.
const mockData = () =>
  ({ signal: false, avg: 0, gain: 1, data: [] }) as unknown as AudioState;
...
interface State {
  loaded: boolean;
  clicked: boolean;
  audio: { drums: AudioState; snare: AudioState; synth: AudioState };
  track: Track;
  api: {
    loaded: () => Promise<void>;
    start: () => void;
