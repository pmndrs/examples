...
type BustGLTFResult = GLTF & {
  materials: { inner: THREE.MeshStandardMaterial };
};
...
function Explosion({
  beat,
  ...props
}: ThreeElements["group"] & { beat: number }) {
  ...
  const play = (_size?: number) =>
    Object.keys(actions).forEach((key) => {
      // three's .d.ts requires `repetitions`, but the original call omits it
      // (repetitions ends up undefined, same as calling setLoop with 1 arg)
      actions[key]!.setLoop(THREE.LoopOnce, undefined as unknown as number)
        .stop()
        .reset();
      actions[key]!.play();
    });
  ...
  useFrame(() => {
    ...
    (sphere.current.children[0] as THREE.PointLight).intensity =
      drums.avg * drums.gain * 10 * Math.PI;
  ...
  return (
    <group {...props}>
      ...
      <group scale={[0.05, 0.05, 0.05]}>
        ...
        <instancedMesh
          ref={instance}
          args={[undefined, undefined, originalScene.children.length]}
        >
...
function Graph(props: Omit<ThreeElements["instancedMesh"], "args">) {
  ...
  return (
    <instancedMesh ref={ref} args={[undefined, undefined, 64]} {...props}>
