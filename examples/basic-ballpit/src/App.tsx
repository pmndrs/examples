...
function InstancedSpheres({ count = 200 }: { count?: number }) {
  const { viewport } = useThree();
  const [ref, api] = useSphere<THREE.InstancedMesh>((index) => ({
    mass: data[index].scale * 100,
    // upstream passes a stray 4th element here; cast preserves that literal as-is
    position: [
      4 - Math.random() * 8,
      viewport.height * 3,
      0,
