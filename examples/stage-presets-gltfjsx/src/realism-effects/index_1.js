...
var fragmentShader$4 = `#define GLSLIFY 1
  ...
  #pragma unroll_loop_start
  for(int i=0;i<textureCount;i++){{if(constantBlend){temporalReprojectMix=accumulatedTexel[i].a==0.0 ? 0.0 : blend;}else{temporalReprojectMix=blend;accumulatedTexel[i].a=mix(accumulatedTexel[i].a,0.0,angleMix);if(reset)accumulatedTexel[i].a=0.0;float maxValue=fullAccumulate ? mix(1.0,blend,moveFactor): blend;if(fullAccumulate&&reprojectedUvSpecular[i].x>0.)maxValue=0.975;float roughnessMaximum=0.25;if(reprojectSpecular[i]&&roughness<roughnessMaximum&&(rayLength>10.0e3||reprojectedUvSpecular[i].x<0.)){float maxRoughnessValue=mix(0.5,maxValue,roughness/roughnessMaximum);maxValue=mix(maxValue,maxRoughnessValue,moveFactor);}temporalReprojectMix=min(1.-1./(accumulatedTexel[i].a+1.0),maxValue);}outputColor=mix(inputTexel[i].rgb,accumulatedTexel[i].rgb,temporalReprojectMix);accumulatedTexel[i].a=1./(1.-temporalReprojectMix)-1.;undoColorTransform(outputColor);gOutput[i]=vec4(outputColor,accumulatedTexel[i].a);texIndex++;}}
  #pragma unroll_loop_end
...
var reproject = `#define GLSLIFY 1
  ...
  bool planeDistanceDisocclusionCheck(const vec3 worldPos,const vec3 lastWorldPos,const vec3 worldNormal,const float distFactor){vec3 toCurrent=worldPos-lastWorldPos;float distToPlane=abs(dot(toCurrent,worldNormal));return distToPlane>PLANE_DISTANCE*distFactor;}bool velocityDisocclusionCheck(const vec2 velocity,const vec2 lastVelocity,const float distFactor){return length(velocity-lastVelocity)>VELOCITY_DISTANCE*distFactor;}bool worldDistanceDisocclusionCheck(const vec3 worldPos,const vec3 lastWorldPos,const float distFactor){return length(worldPos-lastWorldPos)>WORLD_DISTANCE*distFactor;}bool validateReprojectedUV(const vec2 reprojectedUv,const vec3 worldPos,const vec3 worldNormal,const bool isHitPoint){if(reprojectedUv.x>1.0||reprojectedUv.x<0.0||reprojectedUv.y>1.0||reprojectedUv.y<0.0)return false;vec2 dilatedReprojectedUv=reprojectedUv;vec2 lastVelocity=vec2(0.0);vec3 lastWorldNormal=vec3(0.0);float lastDepth=0.0;getVelocityNormalDepth(dilatedReprojectedUv,lastVelocity,lastWorldNormal,lastDepth);vec3 lastWorldPos=screenSpaceToWorldSpace(dilatedReprojectedUv,lastDepth,prevCameraMatrixWorld,prevProjectionMatrixInverse);vec3 lastViewPos=(prevViewMatrix*vec4(lastWorldPos,1.0)).xyz;vec3 lastViewDir=normalize(lastViewPos);vec3 lastViewNormal=(prevViewMatrix*vec4(lastWorldNormal,0.0)).xyz;float lastViewAngle=dot(-lastViewDir,lastViewNormal);angleMix=abs(lastViewAngle-viewAngle)*25.;angleMix=mix(0.,angleMix,flatness);angleMix=min(angleMix,1.);float viewZ=abs(getViewZ(depth));float distFactor=1.+1./(viewZ+1.0);if(velocityDisocclusionCheck(velocity,lastVelocity,distFactor))return false;if(planeDistanceDisocclusionCheck(worldPos,lastWorldPos,worldNormal,distFactor))return false;if(worldDistanceDisocclusionCheck(worldPos,lastWorldPos,distFactor))return false;return true;}vec2 reprojectHitPoint(const vec3 rayOrig,const float rayLength){
...
var gbuffer_packing = `#define GLSLIFY 1
  uniform highp sampler2D gBufferTexture;struct Material{highp vec4 diffuse;highp vec3 normal;highp float roughness;highp float metalness;highp vec3 emissive;};const float c_precision=256.0;const float c_precisionp1=c_precision+1.0;highp float color2float(in highp vec3 color){color=clamp(color,0.0,1.0);return floor(color.r*c_precision+0.5)+floor(color.b*c_precision+0.5)*c_precisionp1+floor(color.g*c_precision+0.5)*c_precisionp1*c_precisionp1;}highp vec3 float2color(in highp float value){vec3 color;color.r=mod(value,c_precisionp1)/c_precision;color.b=mod(floor(value/c_precisionp1),c_precisionp1)/c_precision;color.g=floor(value/(c_precisionp1*c_precisionp1))/c_precision;return color;}highp vec2 OctWrap(highp vec2 v){highp vec2 w=1.0-abs(v.yx);if(v.x<0.0)w.x=-w.x;if(v.y<0.0)w.y=-w.y;return w;}highp vec2 encodeOctWrap(highp vec3 n){n/=(abs(n.x)+abs(n.y)+abs(n.z));n.xy=n.z>0.0 ? n.xy : OctWrap(n.xy);n.xy=n.xy*0.5+0.5;return n.xy;}highp vec3 decodeOctWrap(highp vec2 f){f=f*2.0-1.0;highp vec3 n=vec3(f.x,f.y,1.0-abs(f.x)-abs(f.y));highp float t=max(-n.z,0.0);n.x+=n.x>=0.0 ?-t : t;n.y+=n.y>=0.0 ?-t : t;return normalize(n);}highp float packNormal(highp vec3 normal){return uintBitsToFloat(packHalf2x16(encodeOctWrap(normal)));}highp vec3 unpackNormal(highp float packedNormal){return decodeOctWrap(unpackHalf2x16(floatBitsToUint(packedNormal)));}highp vec4 packTwoVec4(highp vec4 v1,highp vec4 v2){highp vec4 encoded=vec4(0.0);encoded.r=uintBitsToFloat(packHalf2x16(v1.rg));encoded.g=uintBitsToFloat(packHalf2x16(v1.ba));encoded.b=uintBitsToFloat(packHalf2x16(v2.rg));encoded.a=uintBitsToFloat(packHalf2x16(v2.ba));return encoded;}void unpackTwoVec4(highp vec4 encoded,out highp vec4 v1,out highp vec4 v2){v1.rg=unpackHalf2x16(floatBitsToUint(encoded.r));v1.ba=unpackHalf2x16(floatBitsToUint(encoded.g));v2.rg=unpackHalf2x16(floatBitsToUint(encoded.b));v2.ba=unpackHalf2x16(floatBitsToUint(encoded.a));}highp vec4 encodeRGBE8(highp vec3 rgb){highp vec4 vEncoded;highp float maxComponent=max(max(rgb.r,rgb.g),rgb.b);highp float fExp=ceil(log2(maxComponent));vEncoded.rgb=rgb/exp2(fExp);vEncoded.a=(fExp+128.0)/255.0;return vEncoded;}highp vec3 decodeRGBE8(highp vec4 rgbe){highp vec3 vDecoded;highp float fExp=rgbe.a*255.0-128.0;vDecoded=rgbe.rgb*exp2(fExp);return vDecoded;}highp float vec4ToFloat(highp vec4 vec){highp uvec4 v=uvec4(vec*255.0);highp uint value=(v.a<<24u)|(v.b<<16u)|(v.g<<8u)|(v.r);return uintBitsToFloat(value);}highp vec4 floatToVec4(highp float f){highp uint value=floatBitsToUint(f);highp vec4 v;v.r=float(value&0xFFu)/255.0;v.g=float((value>>8u)&0xFFu)/255.0;v.b=float((value>>16u)&0xFFu)/255.0;v.a=float((value>>24u)&0xFFu)/255.0;return v;}highp vec4 packGBuffer(highp vec4 diffuse,highp vec3 normal,highp float roughness,highp float metalness,highp vec3 emissive){highp vec4 gBuffer;diffuse=clamp(diffuse,vec4(0.),vec4(0.999999));roughness=clamp(roughness,0.0,1.0);metalness=clamp(metalness,0.0,1.0);gBuffer.r=vec4ToFloat(diffuse);gBuffer.g=packNormal(normal);gBuffer.b=color2float(vec3(roughness,metalness,0.));gBuffer.a=vec4ToFloat(encodeRGBE8(emissive));return gBuffer;}Material getMaterial(sampler2D gBufferTexture,highp vec2 uv){highp vec4 gBuffer=textureLod(gBufferTexture,uv,0.0);highp vec4 diffuse=floatToVec4(gBuffer.r);highp vec3 normal=unpackNormal(gBuffer.g);highp vec3 roughnessMetalness=float2color(gBuffer.b);highp float roughness=roughnessMetalness.r;highp float metalness=roughnessMetalness.g;highp vec3 emissive=decodeRGBE8(floatToVec4(gBuffer.a));return Material(diffuse,normal,roughness,metalness,emissive);}`
...
class GBufferPass extends Pass {
  ...
  setMRTMaterialInScene() {
    ...
    for (const c of this.visibleMeshes) {
      ...
      copyPropsToGBufferMaterial(originalMaterial, mrtMaterial) // todo: implement selection
...
var fragmentShader$3 = `#define GLSLIFY 1
  ...
  envPdf=sampleEquirectProbability(envMapInfo,random.rg,envMisDir);envMisDir=normalize((vec4(envMisDir,0.)*cameraMatrixWorld).xyz);envMisProbability=0.5+dot(envMisDir,viewNormal)*0.5;envMisProbability*=sqrt(mat.roughness);isEnvMisSample=random.a<envMisProbability;envMisMultiplier=1./(1.-envMisProbability);if(isEnvMisSample){envPdf/=1.-envMisProbability;l=envMisDir;calculateAngles(h,l,v,n,NoL,NoH,LoH,VoH);}else{envPdf=0.0001;}
  #else
  envPdf=0.0;envMisMultiplier=1.;
  #endif
  vec3 diffuseRay=isEnvMisSample? envMisDir: cosineSampleHemisphere(viewNormal,random.rg);vec3 specularRay=(isEnvMisSample&&mat.roughness>=0.025)? envMisDir : l;
  #if mode == MODE_SSGI
  ...
  #endif
  return uv;}vec2 BinarySearch(inout vec3 dir,inout vec3 hitPos){float rayHitDepthDifference;vec2 uv;dir*=0.5;hitPos-=dir;for(int i=0;i<refineSteps;i++){uv=viewSpaceToScreenSpace(hitPos);float unpackedDepth=unpackRGBAToDepth(textureLod(depthTexture,uv,0.0));float depth=getViewZ(unpackedDepth);rayHitDepthDifference=depth-hitPos.z;dir*=0.5;hitPos+=rayHitDepthDifference>0.0 ?-dir : dir;}uv=viewSpaceToScreenSpace(hitPos);return uv;}`
...
var fragmentShader$2 = `#define GLSLIFY 1
  ...
  #endif
  roughnessSpecularFactor=centerMat.roughness*centerMat.roughness;float totalWeight=1.;float totalWeight2=1.;vec3 denoisedDiffuse=centerDiffuseGi.rgb;vec3 denoisedSpecular=centerSpecularGi.rgb;centerWorldPos=getWorldPos(centerDepth,vUv);vec4 random=blueNoise();float r=1.+sqrt(random.a)*exp(-(a+a2)*0.01)*radius;float angle=random.r*2.*PI;float s=sin(angle),c=cos(angle);mat2 rm=mat2(c,-s,s,c);rm*=r;for(int i=0;i<8;i++){vec2 offset=rm*poissonDisk[i];vec2 neighborUv=vUv+offset;Neighbor neighborDiffuse=getNeighborWeight(neighborUv,true);Neighbor neighborSpecular=getNeighborWeight(neighborUv,false);evaluateNeighbor(neighborDiffuse.texel,denoisedDiffuse,totalWeight,neighborDiffuse.weight);evaluateNeighbor(neighborSpecular.texel,denoisedSpecular,totalWeight2,neighborSpecular.weight);}denoisedDiffuse/=totalWeight;denoisedSpecular/=totalWeight2;toLinearSpace(denoisedDiffuse);toLinearSpace(denoisedSpecular);gOutput0=vec4(denoisedDiffuse,centerDiffuseGi.a);gOutput1=vec4(denoisedSpecular,centerSpecularGi.a);}`
...
var ssgi_compose = `#define GLSLIFY 1
  uniform sampler2D inputTexture;uniform sampler2D sceneTexture;uniform sampler2D depthTexture;uniform int toneMapping;uniform bool isDebug;
