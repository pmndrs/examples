...
function getPoint(
  ...
  // three types `Vector4#set` with a required `w`, but this deliberately sets only
  // three components (leaving `w` undefined), so the signature is narrowed here.
  const setXYZ = v.set as unknown as (x: number, y: number, z: number) => void;
  setXYZ.call(
    v,
    Math.random() * 2 - 1,
    Math.random() * 2 - 1,
...
class SimulationMaterial extends THREE.ShaderMaterial {
  constructor() {
    ...
    super({
      ...
      // glsl-curl-noise2 (https://github.com/tiagosr/glsl-curl-noise2), MIT License
      ...
      vec3 curl( vec3 p ) { return curlNoise(p); }
      float noise( vec3 p ) { return cnoise(p); }
