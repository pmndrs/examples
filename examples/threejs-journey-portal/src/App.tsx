...
export const App = ({
  ...
  <Canvas camera={{ fov: 45, position: [-4, 2, -4] }}>
    <Sparkles
      count={scale.length}
      // drei types Sparkles' size prop as number | Float32Array, but it accepts a
      // plain array at runtime too (see @react-three/drei/core/Sparkles.js)
      size={scale as unknown as Float32Array}
      position={[0, 0.9, 0]}
      scale={[4, 1.5, 4]}
      speed={0.3}
    />
...
function Model(props: ThreeElements["group"]) {
  const portalMaterial = useRef<InstanceType<typeof PortalMaterial>>(null!);
  const bakedTexture = useTexture(backedImg);
  const { nodes } = useGLTF(portalModel) as unknown as GLTFResult;
...
export const PortalMaterial = shaderMaterial(
  ...
  // Classic Perlin noise ("cnoise", named cnoise3 here to match the call sites below)
