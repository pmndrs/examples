...
const Spaceman = forwardRef(({ children, ...props }, ref) => {
  const { nodes, materials } = useGLTF(astronautModel)
  useLayoutEffect(() => {
...
const Ship = forwardRef((props, ref) => {
  const { nodes, materials } = useGLTF(spaceshipModel)
  useLayoutEffect(() => {
    Object.values(materials).forEach((material) => {
      material.roughness = 0
    })
...
export default function App() {
  ...
  return (
    <Canvas shadows camera={{ position: [0, 1.5, 3] }}>
      ...
      <spotLight position={[5, 0, 5]} intensity={2.5 * Math.PI} decay={0} penumbra={1} angle={0.35} castShadow color="#0c8cbf" />
