...
function Faucet({ name }: { name: string }) {
  const ref = useRef<THREE.Group>(null!);
  const [hovered, hover] = useState<boolean>();
  const { nodes } = useGLTF(faucetModel) as unknown as GLTFResult;
  // Filter out all meshes belonging to a particular faucet
  const meshes = Object.values(nodes).filter(
    (node) => node.isMesh && node.name.startsWith(name),
  );
  useCursor(!!hovered);
  return (
    <group
...
function Faucets(props: ThreeElements["group"]) {
